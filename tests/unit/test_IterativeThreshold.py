...
class TestAbstractIterativeThreshold(unittest.TestCase):
    ...
    def test_increment_fault_tolerance(self):
        ...
        iterative_stats_ft = IterativeThreshold(dim=1, state=state)

        for x in sample[3:] :
            iterative_stats.increment(x)

        logger.info(f' res={iterative_stats_ft.get_stats()}, gt= {gt}')
        self.assertAlmostEqual(gt, iterative_stats_ft.get_stats(), delta=10e-2)
    ...
    def test_increment_multidim_ft(self): 
        delta = 0.1
        sample = np.array([[10.0, 11.0, 12.0, 13.0, 16.0, -12.2], [1.0 + delta, 1.0 + 2.0 * delta, 1.0 + 3 * delta, 13.0, 16.0, -12.2]])
        sample = np.transpose(sample)
        ...
        state = iterative_stats.save_state()
        iterative_stats_ft = IterativeThreshold(dim=2, state=state)

        for x in sample[3:]:
            iterative_stats.increment(x)

        res = np.allclose(gt,iterative_stats_ft.get_stats(), atol=10e-10)
        self.assertTrue(res)