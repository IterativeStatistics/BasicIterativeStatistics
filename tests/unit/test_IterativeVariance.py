...
class TestIterativeVariance(unittest.TestCase):
    ...
    def test_increment_ft(self):
        ...
        iterative_stats_reload = IterativeVariance(dim=1, state=state)
        for x in sample[3:] :
            res.append(x)
            iterative_stats_reload.increment(x)  

        logger.info(f'{iterative_stats.get_stats()}')     
        self.assertAlmostEqual(gt_stats, iterative_stats_reload.get_stats(), delta=10e-2)
        self.assertEqual(iterative_stats_reload.get_stats().shape, (1,))
